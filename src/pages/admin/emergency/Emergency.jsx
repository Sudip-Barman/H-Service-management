	...
	return (
		<div className="space-y-4 sm:space-y-6">
			<div className="flex flex-col justify-between gap-3 sm:gap-4 md:flex-row md:items-end">
				<div>
					<p className="text-[10px] font-bold uppercase tracking-wider text-[#08A6A0] sm:text-xs">Emergency Department</p>
					<h1 className="mt-1 text-xl font-bold tracking-tight text-[#073F42] sm:text-2xl lg:text-3xl">Emergency Patient Care</h1>
					<p className="mt-1 max-w-2xl text-xs sm:text-sm leading-relaxed text-[#789092]">Coordinate triage, treatment, and patient information from one live emergency desk.</p>
				</div>
				<button type="button" onClick={() => setShowAdd(true)} className="inline-flex h-10 sm:h-11 items-center justify-center gap-2 rounded-xl bg-[#08A6A0] px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm transition hover:bg-[#078F8A]">
					<Plus size={16} className="sm:size-[18px]" /> Add Emergency Patient
				</button>
			</div>
			...
			<div className="rounded-xl border border-[#E2EFED] bg-white p-3 shadow-sm sm:rounded-2xl sm:p-5">
				<div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
					<div>
						<h2 className="text-base sm:text-lg font-bold text-[#073F42]">Emergency Patient Queue</h2>
						<p className="mt-0.5 text-xs sm:text-sm text-[#819596]">Review patient condition, triage priority, and current care status.</p>
					</div>
					<div className="flex flex-col gap-2 sm:flex-row">
						<label className="relative block sm:min-w-64">
							<Search size={16} className="absolute left-3 top-3 text-[#819596]" />
							<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search patient or condition" className="h-9 sm:h-10 w-full rounded-lg border border-[#D9E9E7] pl-9 pr-3 text-xs sm:text-sm outline-none focus:border-[#08A6A0]" />
						</label>
						<select value={triageFilter} onChange={(event) => setTriageFilter(event.target.value)} className="h-9 sm:h-10 rounded-lg border border-[#D9E9E7] px-3 text-xs sm:text-sm text-[#31585A] outline-none focus:border-[#08A6A0]">
							<option>All</option><option>Critical</option><option>Urgent</option><option>Stable</option>
						</select>
						<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="h-9 sm:h-10 rounded-lg border border-[#D9E9E7] px-3 text-xs sm:text-sm text-[#31585A] outline-none focus:border-[#08A6A0]">
				...
				<div className="mt-4 space-y-3 md:hidden">
					...
					{filteredPatients.length === 0 && (
						<div className="rounded-xl border border-[#E2EFED] bg-white p-6 text-center text-xs text-[#819596]">
							No emergency patients match the selected filters.
						</div>
					)}
				</div>
			</div>
...
function EmergencyMobileCard({ patient, onSelect, onStatusChange }) {
	return (
		<div className="rounded-xl border border-[#E2EFED] bg-white p-3.5 shadow-sm">
			<div className="flex items-start justify-between gap-2.5">
				<div className="flex items-center gap-2.5 min-w-0">
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#E8F8F6] text-xs font-bold text-[#078F8A]">
						{patient.name.split(" ").map((part) => part[0]).join("").slice(0, 2)}
					</div>
					<div className="min-w-0">
						<h3 className="truncate text-sm font-bold text-[#073F42]">{patient.name}</h3>
						<p className="text-xs text-[#819596]">{patient.id} · {patient.age} yrs · {patient.gender}</p>
					</div>
				</div>
				<Badge value={patient.triage} styles={triageStyles} />
			</div>

			<div className="mt-3 rounded-lg border border-red-100 bg-red-50/50 p-2.5">
				<p className="text-xs font-bold text-red-800">{patient.condition}</p>
				<p className="mt-0.5 truncate text-[11px] text-red-700">{patient.symptoms}</p>
			</div>
			...
			<div className="mt-3 grid grid-cols-2 gap-2">
				<div className="rounded-lg bg-[#FAFDFC] p-2">
					...
					<p className="mt-0.5 truncate text-xs font-semibold text-[#31585A]">{patient.assignedDoctor}</p>
				</div>
				<div className="rounded-lg bg-[#FAFDFC] p-2">
					<span className="text-[10px] font-semibold uppercase tracking-wide text-[#819596]">Location</span>
					<p className="mt-0.5 truncate text-xs font-semibold text-[#31585A]">{patient.room}</p>
				</div>
				<div className="rounded-lg bg-[#FAFDFC] p-2">
					<span className="text-[10px] font-semibold uppercase tracking-wide text-[#819596]">Arrival</span>
					<p className="mt-0.5 truncate text-xs font-semibold text-[#31585A]">{patient.arrivalTime}</p>
				</div>
				<div className="rounded-lg bg-[#FAFDFC] p-2">
					<span className="text-[10px] font-semibold uppercase tracking-wide text-[#819596]">Contact</span>
					<p className="mt-0.5 truncate text-xs font-semibold text-[#31585A]">{patient.emergencyPhone || patient.phone}</p>
				</div>
			</div>
			...
			<div className="mt-3 flex items-center justify-between gap-2 border-t border-[#EAF2F0] pt-2.5">
				<select
					...
					onChange={(event) => onStatusChange(patient.id, event.target.value)}
					className={`rounded-full border px-2.5 py-1 text-xs font-semibold outline-none ${statusStyles[patient.status]}`}
				>
					<option>Awaiting Doctor</option>
				...
				<button
					type="button"
					onClick={onSelect}
					className="inline-flex items-center gap-1 rounded-lg border border-[#D9E9E7] px-3 py-1.5 text-xs font-semibold text-[#31585A] transition hover:border-[#08A6A0] hover:bg-[#E8F8F6] hover:text-[#078F8A]"
				>
					<Eye size={14} /> Details
				</button>
			</div>
...
function SummaryCard({ icon: Icon, label, value, tone }) {
	const tones = {
		teal: "bg-[#E8F8F6] text-[#08A6A0]",
		red: "bg-red-50 text-red-600",
		blue: "bg-blue-50 text-blue-600",
		amber: "bg-amber-50 text-amber-600"
	};
	return (
		<div className="min-w-0 rounded-lg sm:rounded-xl md:rounded-2xl border border-[#E2EFED] bg-white px-2 py-1.5 sm:px-2.5 sm:py-2.5 md:px-4 md:py-4 shadow-sm transition-all hover:shadow-md">
			<div className="flex items-center justify-between gap-1.5">
				<span className={`flex h-6 w-6 sm:h-7 sm:w-7 md:h-10 md:w-10 shrink-0 items-center justify-center rounded-md sm:rounded-lg md:rounded-xl ${tones[tone]} [&>svg]:h-3 [&>svg]:w-3 sm:[&>svg]:h-3.5 sm:[&>svg]:w-3.5 md:[&>svg]:h-5 md:[&>svg]:w-5`}>
					<Icon />
				</span>
				<strong className="text-base sm:text-lg md:text-2xl font-bold leading-none text-[#073F42]">{value}</strong>
			</div>
			<p className="mt-1 sm:mt-1.5 md:mt-3 truncate text-[9px] sm:text-[10px] md:text-xs font-semibold text-[#819596]">{label}</p>
		</div>
	);
}
...
function PatientDetails({ patient, onClose, onStatusChange }) {
	return <div className="fixed inset-0 z-50 flex justify-end bg-[#073F42]/35 backdrop-blur-sm" onMouseDown={onClose}>
		<aside className="h-full w-full max-w-xl overflow-y-auto bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
			<div className="sticky top-0 z-10 flex items-start justify-between border-b border-[#EAF2F0] bg-white px-4 py-4 sm:px-5 sm:py-5"><div><p className="text-xs font-bold uppercase tracking-wide text-[#08A6A0]">{patient.id}</p><h2 className="mt-1 text-lg sm:text-xl font-bold text-[#073F42]">{patient.name}</h2><p className="mt-0.5 text-xs sm:text-sm text-[#819596]">Emergency patient information</p></div><button type="button" onClick={onClose} aria-label="Close patient details" className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-lg border border-[#D9E9E7] text-[#31585A] hover:bg-[#E8F8F6]"><X size={16} /></button></div>
			<div className="space-y-4 sm:space-y-5 p-4 sm:p-5"><div className="flex flex-wrap items-center gap-2"><Badge value={patient.triage} styles={triageStyles} /><select value={patient.status} onChange={(event) => onStatusChange(patient.id, event.target.value)} className={`rounded-full border px-2.5 py-1 text-xs font-semibold outline-none ${statusStyles[patient.status]}`}><option>Awaiting Doctor</option><option>Under Treatment</option><option>Admitted</option><option>Discharged</option></select></div>
				<section><h3 className="mb-2 sm:mb-3 text-xs sm:text-sm font-bold text-[#073F42]">Patient information</h3><div className="grid grid-cols-2 gap-2 sm:gap-3"><DetailItem label="Age / Gender" value={`${patient.age} years · ${patient.gender}`} icon={UserRound} /><DetailItem label="Blood group" value={patient.bloodGroup} icon={Droplets} /><DetailItem label="Phone" value={patient.phone} icon={Phone} /><DetailItem label="Arrival" value={patient.arrivalTime} icon={Clock3} /></div></section>
				<section><h3 className="mb-2 sm:mb-3 text-xs sm:text-sm font-bold text-[#073F42]">Emergency condition</h3><div className="rounded-xl border border-red-100 bg-red-50/60 p-3 sm:p-4"><p className="text-xs sm:text-sm font-bold text-red-800">{patient.condition}</p><p className="mt-1 sm:mt-2 text-xs sm:text-sm leading-relaxed text-red-700">{patient.symptoms}</p></div></section>
				<section><h3 className="mb-2 sm:mb-3 text-xs sm:text-sm font-bold text-[#073F42]">Care assignment</h3><div className="grid grid-cols-2 gap-2 sm:gap-3"><DetailItem label="Doctor" value={patient.assignedDoctor} icon={Stethoscope} /><DetailItem label="Department" value={patient.department} icon={HeartPulse} /><DetailItem label="Location" value={patient.room} icon={MapPin} /><DetailItem label="Allergies" value={patient.allergies} icon={AlertCircle} /></div></section>
				<section className="rounded-xl border border-[#E2EFED] bg-[#F7FBFA] p-3 sm:p-4"><p className="text-[10px] sm:text-xs font-bold uppercase tracking-wide text-[#819596]">Emergency contact</p><p className="mt-1 font-semibold text-xs sm:text-sm text-[#173F41]">{patient.emergencyContact}</p><p className="mt-0.5 text-xs sm:text-sm text-[#31585A]">{patient.emergencyPhone}</p></section>
				<section><h3 className="mb-1.5 sm:mb-2 text-xs sm:text-sm font-bold text-[#073F42]">Clinical notes</h3><p className="rounded-xl border border-[#E2EFED] p-3 sm:p-4 text-xs sm:text-sm leading-relaxed text-[#31585A]">{patient.notes || "No notes recorded."}</p></section>
			</div>
		</aside>
	</div>;
}
